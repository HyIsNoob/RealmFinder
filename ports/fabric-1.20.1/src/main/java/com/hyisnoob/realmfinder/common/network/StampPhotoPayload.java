...
public record StampPhotoPayload(boolean carve, boolean offHand, UUID snapshotId,
                                double eyeX, double eyeY, double eyeZ, float yaw, float pitch, float scale) implements FabricPacket {
    public static final ResourceLocation ID = RealmFinder.id("stamp_photo");
    public static final PacketType<StampPhotoPayload> TYPE = PacketType.create(ID, StampPhotoPayload::read);
